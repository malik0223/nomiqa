    ...
    <div className="mt-8 space-y-12">
      {issuedSecret ? (
        <div className="rounded-xl border border-amber-300 bg-amber-50 p-4 dark:border-amber-700 dark:bg-amber-950">
          ...
          <p className="mt-1 text-xs text-amber-700 dark:text-amber-300">{labels.secretOnce}</p>
          <div className="mt-2 flex items-center gap-2">
            <code className="flex-1 overflow-x-auto rounded bg-white px-2 py-1 text-xs dark:bg-neutral-900" dir="ltr">
              {issuedSecret.value}
            </code>
            <button
              type="button"
              ...
              className="rounded-lg bg-neutral-100 px-2.5 py-1 text-xs font-medium hover:bg-neutral-200 dark:bg-neutral-800 dark:hover:bg-neutral-700"
            >
      ...
      <section>
        ...
        <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400">{labels.keysIntro}</p>
        ...
        {features.publicApi ? (
          <>
            ...
            <ul className="mt-6 space-y-2">
              {apiKeys.length === 0 ? (
                <li className="text-sm text-neutral-500">{labels.empty}</li>
              ) : (
                apiKeys.map((key) => (
                  <li
                    key={key.id}
                  ...
                  >
                    ...
                    {key.revokedAt ? (
                      <span className="text-xs text-neutral-500">{labels.revoked}</span>
                    ) : (
                      <button
                        type="button"
                        disabled={pending}
                        ...
                        className="rounded-lg px-2.5 py-1 text-xs font-medium text-red-600 hover:bg-red-50 disabled:opacity-50 dark:hover:bg-red-950"
                      >
                        {labels.revoke}
                      </button>
                    )}
      ...
      {/* ---------- CRM ---------- */}
      <section>
        <h2 className="text-lg font-semibold">{labels.crm}</h2>
        <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400">{labels.crmIntro}</p>
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {CRM_SYNCABLE_FIELDS.map((field) => (
                    <label key={field} className="flex items-center gap-2 text-xs">
                      <span className="w-32 shrink-0 text-neutral-500">{field}</span>
                      <input
                        name={`map:${field}`}
                        dir="ltr"
                        defaultValue={
                        ...
                        className="w-full rounded-lg border border-neutral-300 px-2 py-1 dark:border-neutral-700 dark:bg-neutral-900"
                      />
                    </label>
                  ))}
                </div>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="rounded-lg bg-neutral-100 px-2.5 py-1 text-xs font-medium hover:bg-neutral-200 disabled:opacity-50 dark:bg-neutral-800 dark:hover:bg-neutral-700"
                        >
                          {labels.syncAll}
                        </button>
                        <button
