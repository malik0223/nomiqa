    ...
    <form action={submit} className="mt-4 space-y-4">
      ...
      <label className="block text-sm">
        ...
        <input
          ...
          className="mt-1 w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 dark:border-neutral-700 dark:bg-neutral-950"
        />
      </label>
      ...
      <label className="flex items-start gap-2 text-sm">
        ...
        <span>
          ...
          {!kit.hidePlatformBadgeAllowed ? (
            <span className="mt-0.5 block text-xs text-neutral-500">{labels.hideBadgeLocked}</span>
          ) : null}
        </span>
      </label>
      ...
      <div className="flex items-center gap-3">
        ...
        {saved ? <span className="text-sm text-brand-600">{labels.saved}</span> : null}
      </div>
    </form>
  );
}
