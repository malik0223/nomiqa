  ...
  return (
    <div className="mt-4 space-y-8">
      ...
      <div>
        ...
        {flat.length === 0 ? (
          <p className="mt-2 text-sm text-faint">{labels.empty}</p>
        ) : (
          <ul className="mt-3 divide-y divide-line rounded-card border border-line bg-surface shadow-sheet">
            {flat.map((department) => (
              <li key={department.id} className="flex items-center gap-3 px-4 py-2.5">
                ...
                >
                  {department.name}
                  {department.code ? (
                    <code className="ms-2 text-xs text-faint">{department.code}</code>
                  ) : null}
                </span>
            ...
            {branches.map((branch) => (
              <li key={branch.id} className="flex items-center gap-3 px-4 py-2.5">
                <span className="flex-1 text-sm">
                  {branch.name}
                  {branch.city ? (
                    <span className="ms-2 text-xs text-faint">{branch.city}</span>
                  ) : null}
                </span>
                <span className="text-xs text-faint">
