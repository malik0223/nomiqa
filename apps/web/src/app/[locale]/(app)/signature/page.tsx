...
export default async function SignaturePage({ params, searchParams }: PageProps) {
  ...
  try {
    ...
    return (
      <main className="mx-auto w-full max-w-3xl px-5 py-8 sm:px-8 sm:py-10">
        <PageHeader eyebrow={t('nav.groups.identity')} title={t('signature.title')} />
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-8 sm:py-10">
      ...
      {!available ? (
        <p className="mt-6 text-sm text-faint">{t('signature.planLimit')}</p>
      ) : cards.length === 0 ? (
      ...
      <section className="mt-16 border-t border-line pt-10">
        <h2 className="font-display text-base font-semibold tracking-tight">{t('signature.backgrounds')}</h2>
        <p className="mt-1 text-sm text-muted">
          {t('signature.backgroundsIntro')}
        </p>

        {!backgroundsAvailable ? (
          <p className="mt-4 text-sm text-faint">{t('signature.planLimit')}</p>
        ) : signature ? (
          <BackgroundStudio
