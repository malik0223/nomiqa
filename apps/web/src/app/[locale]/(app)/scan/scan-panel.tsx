    ...
    <div className="mt-4">
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-sm">
          <span className="mb-1 block text-muted">{labels.event}</span>
          <select
            value={eventId}
        ...
        <label className="text-sm">
          <span className="mb-1 block text-muted">{labels.kind}</span>
          <select
            value={kind}
      ...
      >
        <label className="flex-1 text-sm">
          <span className="mb-1 block text-muted">{labels.qr}</span>
          <input
            name="payload"
            dir="ltr"
        ...
        <button
          type="submit"
          disabled={pending}
          className="mt-6 h-10 rounded-lg bg-surface-2 px-3 text-sm font-medium hover:bg-surface-3 disabled:opacity-50"
        >
          {labels.qrRead}
        </button>
      </form>
...
function ScanCard({
  ...
  if (scan.status === 'pending' || scan.status === 'processing') {
    return (
      <li className="rounded-card border border-line p-4 text-sm text-faint">
        {labels.processing}
      </li>
    );
  }
  ...
  return (
    <li className="rounded-card border border-line bg-surface p-4 shadow-sheet">
      ...
      >
        ...
        <div className="flex gap-2 sm:col-span-2">
          ...
          <button
            ...
            onClick={onDiscard}
            disabled={pending}
            className="rounded-lg px-4 py-2 text-sm font-medium text-danger-500 hover:bg-danger-50 disabled:opacity-50 dark:hover:bg-red-950"
          >
            {labels.discard}
          </button>
        </div>
  ...
  return (
    <label className="text-sm">
      ...
      <input
        ...
        className={
          low
            ? 'w-full rounded-lg border border-amber-400 bg-warning-50 px-3 py-2 dark:border-amber-600 '
