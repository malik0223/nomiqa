...
const PANEL = 'mt-4 rounded-xl border border-neutral-200 p-5 dark:border-neutral-800';
const CONTROL =
  ...
  return (
    <>
      {error ? (
        <p
          role="alert"
          className="mt-4 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-800 dark:border-red-900 dark:bg-red-950 dark:text-red-200"
        >
          {error}
        </p>
      ) : null}
      ...
      <section className={PANEL}>
        <label htmlFor="follow-up-status" className="text-sm font-semibold">
          {labels.status}
        </label>
        <select
          id="follow-up-status"
      ...
      <section className={PANEL}>
        <h2 className="text-sm font-semibold">{labels.tags}</h2>

        {tags.length === 0 ? (
          <p className="mt-2 text-sm text-neutral-500">{labels.newTag}</p>
        ) : (
                  ...
                  <button
                    ...
                    className={
                      selected
                        ...
                        : 'rounded-full border border-neutral-300 px-3 py-1 text-xs hover:bg-neutral-50 dark:border-neutral-700 dark:hover:bg-neutral-900'
                    }
                  >
                    {tag.name}
                  </button>
      ...
      <section className={PANEL}>
        <h2 className="text-sm font-semibold">{labels.notes}</h2>
        ...
        <ul className="mt-4 space-y-3">
          {contact.notes.map((entry) => (
            <li key={entry.id} className="rounded-lg bg-neutral-50 p-3 text-sm dark:bg-neutral-900">
      ...
      <section className={PANEL}>
        <h2 className="text-sm font-semibold">{labels.followUps}</h2>
          ...
          {contact.followUps.map((task) => (
            <li key={task.id} className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className="flex items-center gap-3 text-xs text-neutral-500">
                <time dateTime={task.dueAt}>{new Date(task.dueAt).toLocaleString(locale)}</time>
                {task.status === 'done' ? (
      ...
      <section className="mt-8 rounded-xl border border-red-200 p-5 dark:border-red-900">
        <DeleteContact
          disabled={pending}
